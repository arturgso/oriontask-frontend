...
<template>
    ...
    >
        <div class="flex gap-2.5 items-center pr-9">
            <div :style="{ backgroundColor: dharma.color }" class="h-4 w-4 rounded-md" />
        ...
        <button
            ...
            @click.stop="toggleOpen(dharma.id)"
        >
            <ChevronDown
                :size="18"
                class="transition-transform duration-200"
                :class="isOpen(dharma.id) ? 'rotate-180' : 'rotate-0'"
            />
        </button>

        <div v-if="isOpen(dharma.id)" class="pt-3" @click.stop>
            <EditDharmaForm :dharma="dharma" @success="handleDharmaAction(dharma.id)" />
        </div>
    </div>
</template>
