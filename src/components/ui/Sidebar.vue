...
<template>
  <aside :class="[
    ...
    <button @click="collapsePanel" :class="[
      'absolute text-text-muted',
      closed ? '' : 'right-4',
      ]">
      <PanelLeftClose/>
    </button>
    <div>
      ...
      <div class="flex flex-col gap-3 mt-10 text-text-primary">
        ...
        <div class="flex flex-col gap-3">
          ...
          <NavButton title="Todos os dharmas" :icon="List" text="Todos os dharmas" push="dharmas" :closed="closed" />
          <NavButton title="Meu Perfil" :icon="User" text="Meu Perfil" push="profile" :closed="closed" />
        </div>
      </div>
    </div>
